...
export default function EditFamilyMemberScreen() {
  ...
  useEffect(() => {
    if (member) {
      // Use rawData if available, otherwise try to parse from flattened fields
      const data = member.rawData || member;

      setFirstName(data.firstname || data.firstName || '');
      setOtherNames(data.othernames || '');
      setLastName(data.surname || data.lastName || ''); // Fallback keys just in case
      setPhone(data.fone || data.phoneNo || data.phone || '');
      setEmail(data.email || '');
      setGender((data.gender as any) || '');
      setRelationship(data.relationship || '');

      // Handle Photo
      // If photo is a full URL, use it. If filename, assume we might need a base URL but for now just set it.
      // The Add screen sets a local URI. The Image component handles remote URLs too.
      if (data.photofilename) {
        setPhoto(data.photofilename);
      ...
      if (data.DoB) {
        ...
        if (!isNaN(dateObj.getTime())) {
          ...
          setDobDay(day);
          setDobMonth(months[monthIdx] || '');
        } else {
          // Try parsing YYYY-MM-DD string manually if Date parse fails or timezones mess it up
          const parts = data.DoB.split('-');
          if (parts.length === 3) {
            setDobDay(parts[2]);
            const mIndex = parseInt(parts[1], 10) - 1;
            setDobMonth(months[mIndex] || '');
          }
        }
      }

      // Employment
      setEmploymentStatus((data.employmentStatus as any) || '');
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      haptics.error();
      Alert.alert('Error', err?.data?.message || 'Failed to update member');
    }
  };

  const handleReset = () => {
    // For edit, reset might mean "Reset to original" or "Clear". 
    // But usually Success Modal "Add New" button should probably go back or something.
    // In Add screen: "Add New Family Member" -> resets form.
    // In Edit screen: "Add New" makes no sense.
    // I will change the success modal buttons for Edit context.
  ...
  return (
    <SafeAreaView style={styles.container} edges={['top']}>
        ...
        <View style={styles.footer}>
          {currentStep === 1 ? (
            ...
            <TouchableOpacity style={styles.mainButton} onPress={handleSubmit} disabled={isLoading}>
              {isLoading ? <ActivityIndicator color="#fff" /> : <Text style={styles.mainButtonText}>Save Changes</Text>}
            </TouchableOpacity>
          )}
        </View>
      </KeyboardAvoidingView>
      ...
      <SuccessModal
        ...
        message="Member details have been successfully updated."
        primaryButtonText="Close"
        // Hide secondary button for edit or reuse it to go back
